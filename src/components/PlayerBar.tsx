  ...
  return (
    <div className="controls">
      <div className="row" style={{ gap: 8 }}>
        <button className="btn" onClick={() => player.prev()}>上一首</button>
        <button className="btn" onClick={() => player.toggle()}>{player.isPlaying ? '暂停' : '播放'}</button>
        <button className="btn" onClick={() => player.next()}>下一首</button>
        <button className="btn" onClick={() => player.setShuffle(!player.shuffle)}>{player.shuffle ? '随机' : '顺序'}</button>
        <button className="btn" onClick={toggleRepeat}>{player.repeatMode === 'off' ? '循环关' : player.repeatMode === 'one' ? '单曲循环' : '列表循环'}</button>
      </div>
      ...
      <div className="row" style={{ justifyContent: 'flex-end', gap: 8 }}>
        ...
        <input className="slider" style={{ maxWidth: 160 }} type="range" min={0} max={1} step={0.01} value={player.volume} onChange={e => player.setVolume(parseFloat(e.target.value))} />
        <button className="btn" onClick={() => player.setMuted(!player.muted)}>{player.muted ? '取消静音' : '静音'}</button>
      </div>
    </div>
  )
}
